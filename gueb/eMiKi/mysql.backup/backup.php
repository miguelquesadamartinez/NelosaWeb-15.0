...
if (!$config_exist) {
	echo "<form method=\"post\" action=../backup//%22backup.php?mode=%22.POST_CONFIG_FORM.%22%5C%22>\n";
...
switch ($mode) {
	case POST_DB_SELECT_FORM: {
		...
		echo "<form method=\"post\" action=../backup//%22backup.php?mode=%22.POST_SELECT_TABLES_FORM.%22&num_tables=$num_tables%5C%22>\n";
	...
	case DB_SELECT_FORM: {
		...
		if ($action == "restore") {
			...
			echo "<form method=\"post\" action=../backup//%22backup.php?mode=%22.RESTORE_DATA.%22&filename=$filename&restore_structure=$restore_structure&restore_data=$restore_data%5C%22>\n";
		}
	...
	default: {
		...
		dialog_box("Backup MySQL Data", "Click <a href=../backup//%22backup.php?mode=%22.DB_SELECT_FORM.%22&action=backup%5C%22>here</a> to backup MySQL data.");

		echo "<p />";

		dialog_box("Restore MySQL Data", "<b>Select a file</b><form method=\"post\" enctype=\"multipart/form-data\" action=../backup//%22backup.php?mode=%22.DB_SELECT_FORM.%22&action=restore%5C%22><input type=\"file\" name=\"filename\" /><br /><br /><b>Select MySQL details to restore</b><br /><br /><input type=\"checkbox\" name=\"structure\" checked />&nbsp;Table Structure<br /><input type=\"checkbox\" name=\"data\" checked />&nbsp;Table Data<br /><br /><input type=\"submit\" value=\"Restore\" /></form><p />");

		echo "<p />";

		dialog_box("Delete MySQL Configuration Details", "Click <a href=../backup//%22backup.php?mode=%22.DELETE_CONFIG_FILE.%22%5C%22>here</a> to delete the MySQL configuration details and re-enter them.<p />");
	}
}
...
if ($mode != POST_SELECT_TABLES_FORM) {
	echo "<p><a href=../backup//%22backup.php/%22>Return to Start</a></p>\n";
